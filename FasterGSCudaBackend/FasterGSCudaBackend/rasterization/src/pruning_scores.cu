...
void faster_gs::rasterization::pruning_scores(
    ...
    float* scores, const int n_primitives, const int active_sh_bases,
  ...
  kernels::pruning_scores::apply_depth_ordering_cu<<<
...
#define COMPUTE_SCORES_ARGS                                               \
  resize_instance_buffers, primitive_buffers, tile_buffers, grid, block,  \
      bg_color, scores, memset_stream, n_visible_primitives, n_instances, \
      end_bit, width, height
  if (end_bit <= 16)
    compute_scores<ushort>(COMPUTE_SCORES_ARGS);
  else
    compute_scores<uint>(COMPUTE_SCORES_ARGS);
...
void faster_gs::rasterization::compute_scores(
    ...
    const dim3& grid, const dim3& block, const float3* bg_color, float* scores,
    const cudaStream_t memset_stream, const int n_visible_primitives,
    const int n_instances, const int end_bit, const int width,
    const int height) {
