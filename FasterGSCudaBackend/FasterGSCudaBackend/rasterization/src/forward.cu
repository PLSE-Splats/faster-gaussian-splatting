...
std::tuple<int, int, int> faster_gs::rasterization::forward(
    ...
    std::function<char*(size_t)> resize_bucket_buffers, const float3* means,
    const float3* scales, const float4* rotations, const float* opacities,
    const float3* sh_coefficients_0, const float3* sh_coefficients_rest,
    const float4* w2c, const float3* cam_position, const float3* bg_color,
    float* image, const int n_primitives, const int active_sh_bases,
    const int total_sh_bases, const int width, const int height,
    const float focal_x, const float focal_y, const float center_x,
    const float center_y, const float near_plane, const float far_plane,
  ...
  if constexpr (!config::debug) {
    ...
    cudaMemsetAsync(tile_buffers.instance_ranges, 0, sizeof(uint2) * n_tiles,
                    memset_stream);
  } else
  ...
  kernels::forward::
      preprocess_cu<<<div_round_up(n_primitives, config::block_size_preprocess),
                      config::block_size_preprocess>>>(
          means, scales, rotations, opacities, sh_coefficients_0,
  ...
  cudaMemcpy(&n_visible_primitives, primitive_buffers.n_visible_primitives,
             sizeof(uint), cudaMemcpyDeviceToHost);
  int n_instances;
  cudaMemcpy(&n_instances, primitive_buffers.n_instances, sizeof(uint),
  ...
  kernels::forward::apply_depth_ordering_cu<<<
      ...
      config::block_size_apply_depth_ordering>>>(
      primitive_buffers.primitive_indices.Current(),
      primitive_buffers.n_touched_tiles, primitive_buffers.offset,
      n_visible_primitives);
  CHECK_CUDA(config::debug, "apply_depth_ordering")

  cub::DeviceScan::ExclusiveSum(
      primitive_buffers.cub_workspace, primitive_buffers.cub_workspace_size,
      primitive_buffers.offset, primitive_buffers.offset, n_visible_primitives);
  ...
  // 4Kx4K images beyond that, 32 bit keys are needed and for best performance,
  // we template the remaining rasterization steps note that with c++20 one
  // could use a templated lambda to improve readability here
  int n_buckets, instance_primitive_indices_selector;
#define DIFF_RASTERIZE_ARGS                                               \
  resize_instance_buffers, resize_bucket_buffers, primitive_buffers,      \
      tile_buffers, grid, block, bg_color, image, memset_stream,          \
      n_visible_primitives, n_instances, n_tiles, end_bit, width, height, \
      n_buckets, instance_primitive_indices_selector
  if (end_bit <= 16)
    diff_rasterize<ushort>(DIFF_RASTERIZE_ARGS);
  else
    diff_rasterize<uint>(DIFF_RASTERIZE_ARGS);
...
void faster_gs::rasterization::diff_rasterize(
    ...
    const int n_instances, const int n_tiles, const int end_bit,
    const int width, const int height, int& n_buckets,
  ...
  cub::DeviceRadixSort::SortPairs(
      ...
      instance_buffers.keys, instance_buffers.primitive_indices, n_instances, 0,
      end_bit);
  CHECK_CUDA(config::debug, "cub::DeviceRadixSort::SortPairs (tile)")

  instance_primitive_indices_selector =
      instance_buffers.primitive_indices.selector;

  if constexpr (!config::debug) cudaStreamSynchronize(memset_stream);

  if (n_instances > 0) {
    kernels::forward::extract_instance_ranges_cu<KeyT>
        <<<div_round_up(n_instances,
  ...
  kernels::forward::extract_bucket_counts<<<
      ...
      tile_buffers.instance_ranges, tile_buffers.n_buckets, n_tiles);
  CHECK_CUDA(config::debug, "extract_bucket_counts")

  cub::DeviceScan::InclusiveSum(
      tile_buffers.cub_workspace, tile_buffers.cub_workspace_size,
      tile_buffers.n_buckets, tile_buffers.buckets_offset, n_tiles);
  CHECK_CUDA(config::debug,
             "cub::DeviceScan::InclusiveSum (tile_buffers.n_buckets)")

  cudaMemcpy(&n_buckets, tile_buffers.buckets_offset + n_tiles - 1,
             sizeof(uint), cudaMemcpyDeviceToHost);

  char* bucket_buffers_blob =
      resize_bucket_buffers(required<BucketBuffers>(n_buckets));
  BucketBuffers bucket_buffers =
      BucketBuffers::from_blob(bucket_buffers_blob, n_buckets);
