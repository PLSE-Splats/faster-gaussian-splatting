...
void faster_gs::rasterization::rasterize(
  ...
  if (n_instances > 0) {
    ...
    kernels::inference::stage_instance_data_cu<<<
        ...
        instance_buffers.screen_bounds, instance_buffers.mean2d,
        instance_buffers.conic_opacity, instance_buffers.color, n_instances);
    CHECK_CUDA(config::debug, "stage_instance_data")
  }

  kernels::inference::blend_cu<<<grid, block>>>(
      tile_buffers.instance_ranges, instance_buffers.screen_bounds,
      instance_buffers.mean2d, instance_buffers.conic_opacity,
      instance_buffers.color, bg_color, image, width, height, grid.x, to_chw);
