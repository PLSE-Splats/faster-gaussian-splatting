...
__global__ void extract_bucket_counts(
    ...
    uint* __restrict__ tile_n_buckets, const uint n_tiles) {
...
__global__ void __launch_bounds__(config::block_size_blend)
  ...
  for (int n_buckets_remaining = n_buckets, current_bucket_idx = thread_rank;
       n_buckets_remaining > 0; n_buckets_remaining -= config::block_size_blend,
...
#pragma unroll
  ...
  typedef cub::BlockReduce<uint, config::tile_width,
                           cub::BLOCK_REDUCE_WARP_REDUCTIONS,
                           config::tile_height>
      BlockReduce;
  __shared__ typename BlockReduce::TempStorage temp_storage;
  n_processed_and_used =
      BlockReduce(temp_storage).Reduce(n_processed_and_used, cub::Max());
