...
__global__ void preprocess_backward_cu(
    ...
    const float4* __restrict__ w2c, const float3* __restrict__ cam_position,
    const uint* __restrict__ primitive_n_touched_tiles,
    const float2* __restrict__ grad_mean2d,
    const float* __restrict__ grad_conic, float3* __restrict__ grad_means,
    float3* __restrict__ grad_scales, float4* __restrict__ grad_rotations,
    float* __restrict__ grad_opacities,
    float3* __restrict__ grad_sh_coefficients_0,
    float3* __restrict__ grad_sh_coefficients_rest,
    float* __restrict__ densification_info, const uint n_primitives,
  ...
  const float3 dL_dmean3d_from_color = convert_sh_to_color_backward(
      ...
      mean3d, cam_position[0], primitive_idx, active_sh_bases, total_sh_bases);

  const float4 w2c_r3 = w2c[2];
  const float depth = w2c_r3.x * mean3d.x + w2c_r3.y * mean3d.y +
                      w2c_r3.z * mean3d.z + w2c_r3.w;
  const float4 w2c_r1 = w2c[0];
  const float x = (w2c_r1.x * mean3d.x + w2c_r1.y * mean3d.y +
                   w2c_r1.z * mean3d.z + w2c_r1.w) /
                  depth;
  const float4 w2c_r2 = w2c[1];
  const float y = (w2c_r2.x * mean3d.x + w2c_r2.y * mean3d.y +
                   w2c_r2.z * mean3d.z + w2c_r2.w) /
                  depth;
  ...
  const mat3x3 R =
      convert_quaternion_to_rotation_matrix(raw_rotation, quaternion_norm_sq);
  const mat3x3 RSS = {
  ...
  const float3 jw_r1 = make_float3(j11 * w2c_r1.x + j13 * w2c_r3.x,
                                   j11 * w2c_r1.y + j13 * w2c_r3.y,
                                   j11 * w2c_r1.z + j13 * w2c_r3.z);
  const float3 jw_r2 = make_float3(j22 * w2c_r2.x + j23 * w2c_r3.x,
  ...
  const float a_raw = dot(jwc_r1, jw_r1), b = dot(jwc_r1, jw_r2),
              c_raw = dot(jwc_r2, jw_r2);
  const float kernel_size = proper_antialiasing
                                ? config::dilation_proper_antialiasing
                                : config::dilation;
  ...
  const float3 dL_dconic = make_float3(
      grad_conic[primitive_idx], grad_conic[n_primitives + primitive_idx],
  ...
  if (proper_antialiasing) {
    ...
    const float dL_dopacity =
        dL_dopacity_conv_factor * conv_factor * opacity * (1.0f - opacity);
    grad_opacities[primitive_idx] = dL_dopacity;
    // the remaining part works but causes exploding gradients that lead to lots
    // of degenerate Gaussians
    if constexpr (!config::detach_dilation_proper_antialiasing_from_cov2d) {
      // based on
      // https://github.com/nerfstudio-project/gsplat/blob/65042cc501d1cdbefaf1d6f61a9a47575eec8c71/gsplat/cuda/include/Utils.cuh#L390
      const float3 conic =
          make_float3(c / determinant, -b / determinant, a / determinant);
      const float determinant_conic = conic.x * conic.z - conic.y * conic.y;
      const float dL_dradicand =
          dL_dopacity_conv_factor * opacity / fmaxf(2.0f * conv_factor, 1e-6f);
      const float one_minus_radicand = 1.0f - radicand;
      dL_dcov2d.x += dL_dradicand * (one_minus_radicand * conic.x -
                                     kernel_size * determinant_conic);
      dL_dcov2d.y += dL_dradicand * 2.0f * one_minus_radicand * conic.y;
      dL_dcov2d.z += dL_dradicand * (one_minus_radicand * conic.z -
                                     kernel_size * determinant_conic);
    }
  }
  ...
  const mat3x3_triu dL_dcov3d = {
      jw_r1.x * jw_r1.x * dL_dcov2d.x + 2.0f * jw_r1.x * jw_r2.x * dL_dcov2d.y +
          jw_r2.x * jw_r2.x * dL_dcov2d.z,
      jw_r1.x * jw_r1.y * dL_dcov2d.x +
          (jw_r1.x * jw_r2.y + jw_r1.y * jw_r2.x) * dL_dcov2d.y +
          jw_r2.x * jw_r2.y * dL_dcov2d.z,
      jw_r1.x * jw_r1.z * dL_dcov2d.x +
          (jw_r1.x * jw_r2.z + jw_r1.z * jw_r2.x) * dL_dcov2d.y +
          jw_r2.x * jw_r2.z * dL_dcov2d.z,
      jw_r1.y * jw_r1.y * dL_dcov2d.x + 2.0f * jw_r1.y * jw_r2.y * dL_dcov2d.y +
          jw_r2.y * jw_r2.y * dL_dcov2d.z,
      jw_r1.y * jw_r1.z * dL_dcov2d.x +
          (jw_r1.y * jw_r2.z + jw_r1.z * jw_r2.y) * dL_dcov2d.y +
          jw_r2.y * jw_r2.z * dL_dcov2d.z,
      jw_r1.z * jw_r1.z * dL_dcov2d.x + 2.0f * jw_r1.z * jw_r2.z * dL_dcov2d.y +
          jw_r2.z * jw_r2.z * dL_dcov2d.z,
  };

  // gradient of J * W
  const float3 dL_djw_r1 =
      2.0f * make_float3(jwc_r1.x * dL_dcov2d.x + jwc_r2.x * dL_dcov2d.y,
                         jwc_r1.y * dL_dcov2d.x + jwc_r2.y * dL_dcov2d.y,
                         jwc_r1.z * dL_dcov2d.x + jwc_r2.z * dL_dcov2d.y);
  const float3 dL_djw_r2 =
      2.0f * make_float3(jwc_r1.x * dL_dcov2d.y + jwc_r2.x * dL_dcov2d.z,
  ...
  if (densification_info != nullptr) {
    ...
    const float2 dL_dmean2d_ndc =
        0.5f * make_float2(dL_dmean2d.x * width, dL_dmean2d.y * height);
  ...
  float3 dL_dmean3d_cam =
      make_float3(j11 * dL_dmean2d.x, j22 * dL_dmean2d.y,
  ...
  dL_dmean3d_cam.z += (j11 * (factor_x * x_clipped * dL_dj13 - dL_dj11) +
                       j22 * (factor_y * y_clipped * dL_dj23 - dL_dj22)) /
                      depth;

  // 3d mean gradient from splatting
  const float3 dL_dmean3d_from_splatting =
      make_float3(w2c_r1.x * dL_dmean3d_cam.x + w2c_r2.x * dL_dmean3d_cam.y +
                      w2c_r3.x * dL_dmean3d_cam.z,
                  w2c_r1.y * dL_dmean3d_cam.x + w2c_r2.y * dL_dmean3d_cam.y +
                      w2c_r3.y * dL_dmean3d_cam.z,
                  w2c_r1.z * dL_dmean3d_cam.x + w2c_r2.z * dL_dmean3d_cam.y +
                      w2c_r3.z * dL_dmean3d_cam.z);
  ...
  const float3 dL_dvariance = make_float3(
      R.m11 * R.m11 * dL_dcov3d.m11 + R.m21 * R.m21 * dL_dcov3d.m22 +
          R.m31 * R.m31 * dL_dcov3d.m33 +
          2.0f *
              (R.m11 * R.m21 * dL_dcov3d.m12 + R.m11 * R.m31 * dL_dcov3d.m13 +
               R.m21 * R.m31 * dL_dcov3d.m23),
      R.m12 * R.m12 * dL_dcov3d.m11 + R.m22 * R.m22 * dL_dcov3d.m22 +
          R.m32 * R.m32 * dL_dcov3d.m33 +
          2.0f *
              (R.m12 * R.m22 * dL_dcov3d.m12 + R.m12 * R.m32 * dL_dcov3d.m13 +
               R.m22 * R.m32 * dL_dcov3d.m23),
      R.m13 * R.m13 * dL_dcov3d.m11 + R.m23 * R.m23 * dL_dcov3d.m22 +
          R.m33 * R.m33 * dL_dcov3d.m33 +
          2.0f *
              (R.m13 * R.m23 * dL_dcov3d.m12 + R.m13 * R.m33 * dL_dcov3d.m13 +
               R.m23 * R.m33 * dL_dcov3d.m23));
  const float3 dL_dscale = 2.0f * variance * dL_dvariance;
  grad_scales[primitive_idx] = dL_dscale;

  // rotation gradient
  const mat3x3 dL_dR = {
      2.0f * (RSS.m11 * dL_dcov3d.m11 + RSS.m21 * dL_dcov3d.m12 +
              RSS.m31 * dL_dcov3d.m13),
      2.0f * (RSS.m12 * dL_dcov3d.m11 + RSS.m22 * dL_dcov3d.m12 +
              RSS.m32 * dL_dcov3d.m13),
      2.0f * (RSS.m13 * dL_dcov3d.m11 + RSS.m23 * dL_dcov3d.m12 +
              RSS.m33 * dL_dcov3d.m13),
      2.0f * (RSS.m11 * dL_dcov3d.m12 + RSS.m21 * dL_dcov3d.m22 +
              RSS.m31 * dL_dcov3d.m23),
      2.0f * (RSS.m12 * dL_dcov3d.m12 + RSS.m22 * dL_dcov3d.m22 +
              RSS.m32 * dL_dcov3d.m23),
      2.0f * (RSS.m13 * dL_dcov3d.m12 + RSS.m23 * dL_dcov3d.m22 +
              RSS.m33 * dL_dcov3d.m23),
      2.0f * (RSS.m11 * dL_dcov3d.m13 + RSS.m21 * dL_dcov3d.m23 +
              RSS.m31 * dL_dcov3d.m33),
      2.0f * (RSS.m12 * dL_dcov3d.m13 + RSS.m22 * dL_dcov3d.m23 +
              RSS.m32 * dL_dcov3d.m33),
      2.0f * (RSS.m13 * dL_dcov3d.m13 + RSS.m23 * dL_dcov3d.m23 +
              RSS.m33 * dL_dcov3d.m33)};
  const float4 dL_drotation =
      convert_quaternion_to_rotation_matrix_backward(raw_rotation, dL_dR);
...
__global__ void blend_backward_cu(
    ...
    const float3* __restrict__ bg_color, const float* __restrict__ grad_image,
    ...
    float2* __restrict__ grad_mean2d, float* __restrict__ grad_conic,
  ...
  const uint2 start_pixel_coords = {tile_coords.x * config::tile_width,
...
#pragma unroll
  for (int i = 0; i < config::block_size_blend + 31; ++i) {
    if (i % 32 == 0) {
      ...
      if (local_idx < config::block_size_blend) {
        ...
        if (pixel_coords.x < width && pixel_coords.y < height) {
          color_pixel_w_bg =
              make_float3(image[pixel_idx], image[n_pixels + pixel_idx],
                          image[2 * n_pixels + pixel_idx]);
          grad_color_pixel = make_float3(grad_image[pixel_idx],
        ...
        collected_color_pixel_after_transmittance[lane_idx] =
            make_float4(color_pixel_w_bg - final_transmittance * background -
    ...
    if (valid_primitive && valid_pixel && lane_idx == 0 &&
      ...
      const float4 color_pixel_after_transmittance =
          collected_color_pixel_after_transmittance[current_shmem_index];
      color_pixel_after = make_float3(color_pixel_after_transmittance);
      transmittance = color_pixel_after_transmittance.w;
      const float4 grad_info_pixel =
          collected_grad_info_pixel[current_shmem_index];
    ...
    const float3 dL_dcolor =
        blending_weight * grad_color_pixel * color_grad_factor;
    ...
    const float dL_dalpha_from_color =
        dot(transmittance * color - color_pixel_after * one_minus_alpha_rcp,
            grad_color_pixel);
    ...
    const float3 dL_dconic =
        0.5f * gaussian_grad_helper *
        make_float3(delta.x * delta.x, delta.x * delta.y, delta.y * delta.y);
    dL_dconic_accum += dL_dconic;
    const float2 dL_dmean2d =
        gaussian_grad_helper *
  ...
  if (valid_primitive) {
    ...
    const float dL_dopacity =
        proper_antialiasing ? dL_dopacity_accum
