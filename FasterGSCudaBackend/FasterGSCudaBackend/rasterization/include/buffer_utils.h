...
static void obtain(char*& blob, T*& ptr, size_t count) {
  ...
  size_t offset =
      (reinterpret_cast<size_t>(blob) + alignment - 1) & ~(alignment - 1);
...
struct PrimitiveBuffers {
  ...
  static PrimitiveBuffers from_blob(char*& blob, int n_primitives) {
    ...
    buffers.depth_keys =
        cub::DoubleBuffer<uint>(depth_keys_current, depth_keys_alternate);
    ...
    buffers.primitive_indices = cub::DoubleBuffer<uint>(
        primitive_indices_current, primitive_indices_alternate);
    ...
    cub::DeviceRadixSort::SortPairs(nullptr, sorting_workspace_size,
                                    buffers.depth_keys,
                                    buffers.primitive_indices, n_primitives);
    buffers.cub_workspace_size =
        max(buffers.cub_workspace_size, sorting_workspace_size);
...
struct InstanceBuffers {
  ...
  static InstanceBuffers from_blob(char*& blob, int n_instances, int end_bit) {
    ...
    buffers.primitive_indices = cub::DoubleBuffer<uint>(
        primitive_indices_current, primitive_indices_alternate);
    cub::DeviceRadixSort::SortPairs(nullptr, buffers.cub_workspace_size,
...
struct InferenceInstanceBuffers {
  ...
  static InferenceInstanceBuffers from_blob(char*& blob, int n_instances,
                                            int end_bit) {
    InferenceInstanceBuffers buffers;
    InstanceBuffers<KeyT> base_buffers =
        InstanceBuffers<KeyT>::from_blob(blob, n_instances, end_bit);
    ...
    obtain(blob, buffers.screen_bounds, n_instances);
...
struct TileBuffers {
  ...
  static TileBuffers from_blob(char*& blob, int n_tiles) {
    ...
    obtain(blob, buffers.final_transmittances,
           n_tiles * config::block_size_blend);
    ...
    obtain(blob, buffers.final_transmittances,
           n_tiles * config::block_size_blend);
    cub::DeviceScan::InclusiveSum(nullptr, buffers.cub_workspace_size,
...
struct BucketBuffers {
  ...
  static BucketBuffers from_blob(char*& blob, int n_buckets) {
    ...
    obtain(blob, buffers.color_transmittance,
           n_buckets * config::block_size_blend);
    return buffers;
  }
};
