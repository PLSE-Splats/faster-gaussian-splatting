...
__global__ void preprocess_cu(
  ...
  if (config::original_opacity_interpretation &&
      opacity < config::min_alpha_threshold)
    active = false;
  ...
  float3 cov2d =
      make_float3(dot(jwc_r1, jw_r1), dot(jwc_r1, jw_r2), dot(jwc_r2, jw_r2));
  ...
  const float3 conic = make_float3(
      cov2d.z / determinant, -cov2d.y / determinant, cov2d.x / determinant);
  ...
  const float2 mean2d =
      make_float2(x * focal_x + center_x, y * focal_y + center_y);

  // compute bounds
  const float power_threshold =
      config::original_opacity_interpretation
          ? logf(opacity * config::min_alpha_threshold_rcp)
  ...
  primitive_geometry[primitive_idx] = make_float4(
      mean2d.x, mean2d.y,
      __uint_as_float(pack_bounds_xy(static_cast<ushort>(screen_bounds.x),
                                     static_cast<ushort>(screen_bounds.y))),
      __uint_as_float(pack_bounds_xy(static_cast<ushort>(screen_bounds.z),
...
// based on
// https://github.com/r4dl/StopThePop-Rasterization/blob/d8cad09919ff49b11be3d693d1e71fa792f559bb/cuda_rasterizer/stopthepop/stopthepop_common.cuh#L325
...
__global__ void create_instances_cu(
  ...
  const bool compute_cooperatively =
      active && instance_count > config::n_sequential_threshold;
  ...
  for (uint n = 0; n < n_remaining_threads && n < warp_size; n++) {
    ...
    for (uint i = 0; i < n_iterations; i++) {
      ...
      if (write) {
        const uint write_offset = current_write_offset_coop +
                                  __popc(write_ballot & previous_lanes_mask);
        const uint tile_idx = tile_y * grid_width + tile_x;
        const KeyT instance_key = static_cast<KeyT>(tile_idx);
...
__global__ void extract_instance_ranges_cu(
    ...
    uint2* __restrict__ tile_instance_ranges, const uint n_instances) {
...
__global__ void __launch_bounds__(config::block_size_blend)
    compute_scores_cu(const uint2* __restrict__ tile_instance_ranges,
                      const uint* __restrict__ instance_primitive_indices,
                      const float4* __restrict__ primitive_geometry,
                      const float4* __restrict__ primitive_conic_opacity,
                      const float4* __restrict__ primitive_color_rgba,
                      const float3* __restrict__ bg_color,
                      float* __restrict__ scores, const uint width,
  ...
  for (int n_points_remaining = tile_range.y - tile_range.x,
    ...
    if (current_fetch_idx < tile_range.y) {
      ...
      const float4 geometry_record = primitive_geometry[primitive_idx];
      collected_mean2d[thread_rank] =
          make_float2(geometry_record.x, geometry_record.y);
  ...
  // re-run blending to compute the partial derivatives required for the pruning
  // scores
  const float3 grad_color_pixel = make_float3(1.0f);
  const float grad_alpha_common =
      transmittance * -dot(grad_color_pixel, bg_color[0]);
  float3 color_pixel_after =
      color_pixel;  // let's just rename this for readability
  transmittance = 1.0f;
  done = !inside;
  // collaborative loading and processing
  for (int n_points_remaining = tile_range.y - tile_range.x,
    ...
    for (int j = 0; !done && j < current_batch_size; ++j) {
      ...
      const float3 color = make_float3(collected_color[j]);
      ...
      const float dL_dalpha_from_color =
          dot(transmittance * color - color_pixel_after * one_minus_alpha_rcp,
              grad_color_pixel);
      const float dL_dalpha_from_alpha =
          grad_alpha_common * one_minus_alpha_rcp;
