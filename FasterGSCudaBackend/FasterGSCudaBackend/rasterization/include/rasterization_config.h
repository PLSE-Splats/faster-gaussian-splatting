...
DEF bool detach_dilation_proper_antialiasing_from_cov2d =
    true;  // note: detaching leads to more stable gradients and less degenerate
           // Gaussians
DEF float min_cov2d_determinant =
    1e-6f;  // note: backward pass includes factor of 1 / (determinant^2)
DEF bool original_opacity_interpretation =
    true;  // whether to interpret opacity as part of the Gaussian as in 3DGS or
...
#define TRUNCATION_MODE \
  0  // 0: 3.33 sigma (original), 1: 1 sigma, 2: 2 sigma, 3: 3 sigma, 4: 4 sigma
#if TRUNCATION_MODE == 0
DEF float min_alpha_threshold_rcp = 255.0f;
DEF float min_alpha_threshold =
    1.0f / min_alpha_threshold_rcp;              // 0.00392156862
...
static_assert(
    ...
    "one sigma truncation not possible with original opacity interpretation");
DEF float min_alpha_threshold_rcp = 2.71828182846f;
DEF float min_alpha_threshold =
    1.0f / min_alpha_threshold_rcp;    // 0.36787944117
DEF float max_power_threshold = 1.0f;  // ln(min_alpha_threshold_rcp)
#elif TRUNCATION_MODE == 2
static_assert(
    !original_opacity_interpretation,
    "two sigma truncation not possible with original opacity interpretation");
DEF float min_alpha_threshold_rcp = 7.38905609893f;
DEF float min_alpha_threshold =
    1.0f / min_alpha_threshold_rcp;    // 0.13533528323
DEF float max_power_threshold = 2.0f;  // ln(min_alpha_threshold_rcp)
#elif TRUNCATION_MODE == 3
static_assert(
    !original_opacity_interpretation,
    "three sigma truncation not possible with original opacity interpretation");
DEF float min_alpha_threshold_rcp = 90.0171313005f;
DEF float min_alpha_threshold =
    1.0f / min_alpha_threshold_rcp;    // 0.01110899653
...
DEF float min_alpha_threshold =
    1.0f / min_alpha_threshold_rcp;    // 0.00033546262
...
DEF int block_width_blend = 16;
DEF int block_height_blend = 16;
DEF int block_size_blend = block_width_blend * block_height_blend;
// Tile size constants.
DEF int tile_width = 32;
DEF int tile_height = 32;
DEF int tile_size = tile_width * tile_height;
DEF int n_sequential_threshold = 4;
DEF int blocks_per_tile_row = tile_width / block_width_blend;
DEF int blocks_per_tile_column = tile_height / block_height_blend;
// warp size constants
